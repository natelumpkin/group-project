...
def get_all_followers(id):
    ...
    for follower in current_user_followers_query:
        follower_user = follower.to_dict_less()
        follower_user['following'] = follower in following_list
        current_user_followers["Followers"].append(follower_user)
...
def get_all_following(id):
    ...
    current_user_following_query = current_user.following.all()

    current_user_following = {
        "Following": [],
        "totalFollowing": len(current_user_following_query)
    }

    for following in current_user_following_query:
        print(following.to_dict())
        current_user_following['Following'].append(following.to_dict_less())

    return current_user_following
